...
- (void)viewDidLoad
{
    ...
    self.prefContents = @[
        @[
            ...
            @{
                ...
                @"enableCondition": ^BOOL(){
                    ...
                    int count =
                        [NSFileManager.defaultManager
                            contentsOfDirectoryAtPath:demoPath
                            error:nil]
                        .count;
                ...
                @"action": ^void(){
                    ...
                    if(
                        [NSFileManager.defaultManager
                            removeItemAtPath:demoPath
                            error:&error]
                    ) {
                        [NSFileManager.defaultManager
                            createDirectoryAtPath:demoPath
                            withIntermediateDirectories:YES
                            attributes:nil
                            error:nil];

                        [NSFileManager.defaultManager
                            changeCurrentDirectoryPath:demoPath];

                        if (getenv("DEMO_LOCK")) {
                            [(LauncherNavigationController *)
                                self.navigationController
                                fetchLocalVersionList];
                        }

                    } else {
                        NSLog(
                            @"Error in erase_demo_data: %@",
                            error
                        );
            ...
            @{
                ...
                @"action": ^(BOOL enabled){
                    ...
                    if (enabled) {
                        ...
                        [self.presentingViewController
                            performSelector:
                                @selector(switchToInternalDisplay)];
                    }
                }
            },
        ...
        @[
            ...
            @{
                ...
                @"enableCondition": ^BOOL(){
                    return
                        realUIIdiom != UIUserInterfaceIdiomTV;
                }
            },
    ...
    if (NSProcessInfo.processInfo.isMacCatalystApp) {
        UIButton *closeButton =
            [UIButton buttonWithType:UIButtonTypeClose];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.navigationController == nil) {
        [self.presentingViewController
            performSelector:
                @selector(updatePreferenceChanges)];
    }
}
