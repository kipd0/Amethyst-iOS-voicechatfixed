...
+ (id)defaultPrefForGlobal:(BOOL)global {
        ...
        @"control": @{
            ...
            @"control_safe_area":
                UIApplication.sharedApplication
                    ? NSStringFromUIEdgeInsets(getDefaultSafeArea())
                    : @"",
        ...
        @"java": @{
            ...
            @"allocated_memory":
                [NSNumber numberWithFloat:
                    roundf(
                        (NSProcessInfo.processInfo.physicalMemory / 1048576)
                        * 0.25
                    )
                ]
        }.mutableCopy,
    ...
    if (global) {
        ...
        NSDictionary *general = @{
            ...
            @"hidden_sidebar":
                @(realUIIdiom == UIUserInterfaceIdiomPhone),

            @"appicon": @"AppIcon-Light"
        };

        [defaults[@"general"]
            addEntriesFromDictionary:general];
        ...
        defaults[@"debug"] = @{
            ...
            @"debug_ipad_ui":
                @(realUIIdiom == UIUserInterfaceIdiomPad),
...
+ (id)getPreference:(NSString *)key
               from:(NSDictionary *)pref {

    for (NSDictionary *section in pref.allValues) {
        if (
            [section isKindOfClass:NSDictionary.class]
            && section[key]
        ) {
            return section[key];
        }
...
+ (id)getOldLayoutPreference:(NSString *)key
                        from:(NSDictionary *)pref {
    ...
    id value =
        [self getPreference:key from:pref];

    if (!value) {
        NSLog(
            @"[PLPreferences] Migrator could not find preference %@",
            key
        );
    }
...
- (id)initWithAutomaticMigrator {
    ...
    NSMutableDictionary *pref =
        [NSMutableDictionary
            dictionaryWithContentsOfFile:self.globalPath];
    ...
    NSMutableDictionary *oldPref =
        [NSMutableDictionary
            dictionaryWithContentsOfFile:oldPath];
    ...
    if (pref || !oldPref[@"env_vars"]) {
        ...
        NSDebugLog(
            @"[PLPreferences] Migrating to %@",
            self.globalPath.lastPathComponent
        );
        ...
        for (NSString *section in self.globalPref.allKeys) {
            for (
                NSString *key
                in self.globalPref[section].allKeys
            ) {
                id value =
                    [PLPreferences
                        getOldLayoutPreference:key
                        from:oldPref];

                if (value) {
                    self.globalPref[section][key] =
                        value;
                }
            }
        }
...
- (id)setDefaultsForPref:(NSMutableDictionary *)pref
                  global:(BOOL)global {
    ...
    > *defaults =
        [PLPreferences
            defaultPrefForGlobal:global];

    if (!pref) {
        NSLog(
            @"[PLPreferences] Initializing default values "
             "for %@ preferences",
            global ? @"global" : @"isolated"
        );
    ...
    for (NSString *section in defaults.allKeys) {
        if (!pref[section]) {
            NSDebugLog(
                @"[PLPreferences] Set default values "
                 "for section %@",
                section
            );
        ...
        for (
            ...
            NSDebugLog(
                @"[PLPreferences] Set default vaule: %@: %@",
                key,
                value
            );
...
- (void)toggleIsolationForced:(BOOL)force {
    NSMutableDictionary *instancePref =
        [NSMutableDictionary
            dictionaryWithContentsOfFile:self.instancePath];

    if (
        force
        ||
        [instancePref[@"internal"][@"isolated"]
            boolValue]
    ) {
        NSLog(
            @"[PLPreferences] Using isolated preferences from %@",
            self.instancePath.stringByResolvingSymlinksInPath
        );
        ...
        if (!instancePref) {
            ...
            for (
                NSString *section
                in self.instancePref
            ) {
                for (
                    NSString *key
                    in self.instancePref[section].allKeys
                ) {
                    self.instancePref[section][key] =
                        self.globalPref[section][key];
                }
            }
        }
        ...
        self.instancePref[@"internal"][@"isolated"] =
            @YES;
        ...
        NSLog(
            @"[PLPreferences] Using global preferences"
        );
...
- (id)getObject:(NSString *)key {
    ...
    if (!value) {
        NSLog(
            @"[PLPreferences] Getter could not find preference %@",
            key
        );
    }
...
- (void)reset {
    if (self.instancePref) {
        [NSFileManager.defaultManager
            removeItemAtPath:self.instancePath
            error:nil];
...
- (void)saveGlobalPref {
    [self.globalPref
        writeToFile:self.globalPath
        atomically:YES];
}

- (void)saveInstancePref {
    [self.instancePref
        writeToFile:self.instancePath
        atomically:YES];
}
