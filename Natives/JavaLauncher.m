...
void init_loadCustomJvmFlags(int* argc, const char** argv) {
    ...
    for (NSString *arg in [jvmargs componentsSeparatedByString:@" -"]) {
        ...
        argv[*argc] =
            [@"-" stringByAppendingString:jvmarg].UTF8String;
...
int launchJVM(
    ...
    // refresh JIT flags right after loading env
    DeviceGetJITFlags(YES);

    BOOL requiresTXMWorkaround =
        DeviceHasJITFlags(
            JIT_FLAG_FORCE_MIRRORED |
            JIT_FLAG_HAS_TXM
        );

    BOOL jit26AlwaysAttached =
        getPrefBool(@"debug.debug_always_attached_jit");
    ...
    if (requiresTXMWorkaround) {
        ...
        if ((uint32_t)result != 0x690000E0) {
            ...
            NSString *inBundleScriptPath =
                [NSBundle.mainBundle
                    pathForResource:@"UniversalJIT26"
                    ofType:@"js"];

            NSString *lcAppInfoPath =
                [NSBundle.mainBundle.bundlePath
                    stringByAppendingPathComponent:@"LCAppInfo.plist"];

            NSMutableDictionary *lcAppInfo =
                [NSMutableDictionary
                    dictionaryWithContentsOfFile:lcAppInfoPath];

            if(lcAppInfo) {
                // if this is inside LiveContainer, we assign script ourselves
                // and prompt user to restart Amethyst
                lcAppInfo[@"jitLaunchScriptJs"] =
                    [[NSData dataWithContentsOfFile:inBundleScriptPath]
                        base64EncodedStringWithOptions:0];

                if([lcAppInfo writeToFile:lcAppInfoPath atomically:YES]) {
                    showDialog(
                        localize(@"Error", nil),
                        @"Amethyst was launched with a legacy script. "
                         "We have updated the script to Universal, "
                         "please restart LiveContainer to continue."
                    );
            ...
            [NSFileManager.defaultManager
                copyItemAtPath:inBundleScriptPath
                toPath:[
            ...
            showDialog(
                localize(@"Error", nil),
                @"Support for legacy script has been removed. "
                 "Please switch to Universal JIT script. "
                 "To import it, long-press on Amethyst when enabling JIT "
                 "in StikDebug and tap \"Assign Script\", then go to "
                 "Amethyst's Documents directory and pick it. "
                 "(on sideloaded StikDebug, the builtin script is named "
                 "Amethyst-MeloNX.js)"
            );
        ...
        JIT26SendJITScript(
            [NSString stringWithContentsOfFile:
                [NSBundle.mainBundle
                    pathForResource:@"UniversalJIT26Extension"
                    ofType:@"js"]
            ]
        );
    ...
    if (!requiresTXMWorkaround || jit26AlwaysAttached) {
        if (jit26AlwaysAttached) {
            // Only allow StikDebug to catch our breakpoints to prevent stutters
            task_set_exception_ports(
                mach_task_self(),
        ...
        // Activate Library Validation bypass for external runtime and dylibs
        // (JNA, etc)
        init_bypassDyldLibValidation();

    } else {
        NSLog(
            @"[DyldLVBypass] Hook disabled! "
             "Loading unsigned dylib will cause code signature error."
        );
    }
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        ...
        int preferredJavaVersion =
            [PLProfiles
                resolveKeyForCurrentProfile:@"javaVersion"]
                .intValue;

        if (preferredJavaVersion > 0) {
            if (minVersion > preferredJavaVersion) {
                NSLog(
                    @"[JavaLauncher] Profile's preferred Java version "
                     "(%d) does not meet the minimum version (%d), "
                     "dropping request",
                ...
                NSDebugLog(
                    @"[PLProfiles] Applying javaVersion"
                );
        ...
        NSString *renderer =
            [PLProfiles
                resolveKeyForCurrentProfile:@"renderer"];

        NSLog(
            @"[JavaLauncher] RENDERER is set to %@\n",
            renderer
        );
        ...
        gameDir =
            [NSString
                stringWithFormat:
                    ...
                    [PLProfiles
                        resolveKeyForCurrentProfile:@"gameDir"]
            ]
    ...
    NSLog(
        @"[JavaLauncher] Looking for Java %d or later",
        minVersion
    );
    ...
    if (javaHome == nil) {
        ...
        showDialog(
            ...
            [NSString
                stringWithFormat:
                    localize(
                        @"java.error.missing_runtime",
                        nil
                    ),
                    isExecuteJar
                        ? [launchTarget lastPathComponent]
        ...
        NSString *dest =
            [NSString
                stringWithFormat:
                    @"%@/lib/libawt_xawt.dylib",
                    javaHome];

        NSString *source =
            [NSString
                stringWithFormat:
                    @"%@/Frameworks/libawt_xawt.dylib",
                    NSBundle.mainBundle.bundlePath];
        ...
        if (error) {
            NSLog(
                @"[JavaLauncher] Copy libawt_xawt.dylib failed: %@",
    ...
    NSLog(
        @"[JavaLauncher] JAVA_HOME has been set to %@",
        javaHome
    );
    ...
    if (getPrefBool(@"java.auto_ram")) {
        ...
        allocmem =
            roundf(
                (NSProcessInfo.processInfo.physicalMemory >> 20)
                * autoRatio
            );
    ...
    NSLog(
        @"[JavaLauncher] Max RAM allocation is set to %d MB",
        allocmem
    );
        ...
        if (getEntitlementValue(
            ...
            showDialog(
                ...
                @"Insufficient contiguous virtual memory space. "
                 "Lower memory allocation and try again."
            );
            ...
            showDialog(
                localize(@"Error", nil),
                @"Insufficient contiguous virtual memory space. "
                 "Increased Memory Limit entitlement is missing, "
                 "please add it via GetMoreRam app."
            );
        }
    ...
    // Setup options.txt
    [MinecraftOptionUtils
        setupOptionsAtGameDir:gameDir];
    ...
    if (!launchJar) {
        margv[++margc] =
            "-Djava.system.class.loader="
             "net.kdt.pojavlaunch.PojavClassLoader";
    }
    ...
    margv[++margc] =
        [NSString
            stringWithFormat:
                @"-DUIScreen.maximumFramesPerSecond=%d",
                (int)UIScreen.mainScreen.maximumFramesPerSecond]
            .UTF8String;
    ...
    if(getPrefBool(@"general.cosmetica")) {
        margv[++margc] =
            [NSString
                stringWithFormat:
                    @"-javaagent:%@/arc_dns_injector.jar=23.95.137.176",
                    librariesPath]
                .UTF8String;
    }

    // Simple Voice Chat compatibility patch
    if(getPrefBool(@"video.fix_simple_voice_chat_mod")) {
        margv[++margc] =
            [NSString
                stringWithFormat:
                    @"-javaagent:%@/patchsvc.jar=",
                    librariesPath]
                .UTF8String;
    }
    ...
    BOOL isJava8 =
        [fm fileExistsAtPath:libjlipath8];
    ...
    margv[++margc] =
        "-Dcacio.font.fontmanager="
         "sun.awt.X11FontManager";

    margv[++margc] =
        "-Dcacio.font.fontscaler="
         "sun.font.FreetypeFontScaler";

    margv[++margc] =
        [NSString
            stringWithFormat:
                @"-Dcacio.managed.screensize=%dx%d",
                width,
                height]
            .UTF8String;
    ...
    if (isJava8) {
        ...
        margv[++margc] =
            "-Dawt.toolkit="
             ...
             "cacio.ctc.CTCToolkit";

        margv[++margc] =
            "-Djava.awt.graphicsenv="
             "com.github.caciocavallosilano."
             "cacio.ctc.CTCGraphicsEnvironment";
        ...
        // TODO: workaround, will be removed once the startup part
        // works without PLaunchApp
        margv[++margc] =
            "--add-exports="
             "cpw.mods.bootstraplauncher/"
             "cpw.mods.bootstraplauncher=ALL-UNNAMED";
    }

    // Add Caciocavallo bootclasspath
    NSString *cacio_classpath =
        [NSString
            stringWithFormat:
                @"-Xbootclasspath/%s",
                isJava8 ? "p" : "a"];
    ...
    NSArray *files =
        [fm
            contentsOfDirectoryAtPath:cacio_libs_path
            error:nil];
    ...
    margv[++margc] =
        cacio_classpath.UTF8String;

    // FIXME: the JVM arg is deprecated;
    // and it is currently broken for Java 25
    if (
        !getenv("JVM_KEEP_UseCompressedClassPointers")
        ||
        !getEntitlementValue(
            @"com.apple.developer.kernel.extended-virtual-addressing"
        )
    ) {
        // In jailed environment, where extended virtual addressing
        // entitlement isn't present (for free dev account),
        // allocating compressed space fails.
        //
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        for (
            NSString *arg
            in launchTarget[@"arguments"][@"jvm_processed"]
        ) {
            margv[++margc] =
                arg.UTF8String;
        }
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        margv[++margc] =
            [launchTarget[@"id"] UTF8String];

    } else {
        margv[++margc] =
            [launchTarget UTF8String];
    }
    ...
    return pJLI_Launch(
        ...
        // sizeof(const_appclasspath) / sizeof(char *),
        // const_appclasspath,

        // These values are ignored in Java 17,
        // so keep it anyways
        "1.8.0-internal",
        "1.8",
